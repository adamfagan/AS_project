...
const FORM_ENDPOINT = "http://localhost:8080/predict-flowers";

const Flowers = () => {
    const [prediction, setPrediction] = useState({});

    const uploadPicture = (e) => {
      var outputFlowers = document.getElementById("outputFlowers");
      outputFlowers.classList.remove("hidden");
      outputFlowers.classList.add("block");
      outputFlowers.src = URL.createObjectURL(e.target.files[0]);
      outputFlowers.onload = function () {
        URL.revokeObjectURL(outputFlowers.src);
      };
      var predictFlowers = document.getElementById("predictFlowers");
      predictFlowers.classList.remove("hidden");
      predictFlowers.classList.add("block");
    };
    ...
    return (
      <main className="flex flex-col items-center gap-y-8 mt-20 bg-yellow-200 rounded-xl">
      ...
      <form onSubmit={setImageAction} encType="multipart/form-data">
        ...
        <div className="p-4" id="result">
          <div className="flex flex-row">
            <h1 className="text-lg font-semibold text-center">Druh kvetu:</h1>
        ...
        <button
          className="hidden bg-white m-10 text-black rounded-lg shadow-lg h-10 rounded-lg tracking-wide uppercase border border-blue cursor-pointer hover:bg-black hover:text-white transform cursor:pointer hover:scale-105 cursor-pointer transition duration-500 linear w-60"
          type="submit"
          id="predictFlowers"
        >
          Predikovať
